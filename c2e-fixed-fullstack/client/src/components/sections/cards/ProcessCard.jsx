...
function ProcessCard({ step, icon: Icon, title, description, index = 0 }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      variants={fadeUp}
      className="group flex flex-col items-center text-center gap-2 sm:gap-3 rounded-2xl sm:rounded-[1.5rem] bg-white border border-slate-100 shadow-sm px-3 sm:px-4 py-5 sm:py-8 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1 hover:shadow-md h-full"
    >
